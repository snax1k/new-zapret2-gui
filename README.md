...
| `release-v0.0.8/` | текущая собранная версия с README |
| `src-tauri/` | остаток раннего варианта хоста, в сборке не участвует |

В дереве хранится только последняя сборка. Каталоги `release-v0.0.2` …
`release-v0.0.7` удалены, но остались в истории git — при необходимости
достаются оттуда:
